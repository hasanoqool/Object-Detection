...
class ObjectDetector(object):
    def __init__(self,classifier,preprocess_fn=lambda x: x,input_size=(299, 299),confidence=0.98,window_step_size=16,pyramid_scale=1.5,roi_size=(200, 150),nms_threshold=0.3):
        self.classifier = classifier #trained network we'll use to classify each window
        self.preprocess_fn = preprocess_fn #process each window prior to passing it to the classifier
        self.input_size = input_size
        self.confidence = confidence #minimum probability we'll allow detections to have in order to consider them valid.
    ...
    def pyramid(self, image):
        ...
        while True:
            ...
            if (image.shape[0] < self.roi_size[1] or  image.shape[1] <
            self.roi_size[0]):
                break
            yield image
...
def main():
    ...
    #load image and run ObjectDetector
